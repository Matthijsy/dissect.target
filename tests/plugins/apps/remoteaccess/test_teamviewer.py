...
def test_teamviewer_paths(target_win_users: Target, fs_win: VirtualFilesystem) -> None:
    ...
    paths = list(target_win_users.teamviewer.get_paths())
    assert len(paths) == 1
    assert str(paths[0]) == "/sysvol/Program Files/TeamViewer/TestTeamviewer.log"
